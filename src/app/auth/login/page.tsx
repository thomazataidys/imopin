...
function LoginForm() {
  ...
  // Captura erros de login vindos do NextAuth na URL
  useEffect(() => {
    const errorParam = searchParams.get('error')
    if (errorParam === 'CredentialsSignin') {
      setGeneralError('E-mail ou senha incorretos, ou e-mail ainda não confirmado.')
    } else if (errorParam) {
      setGeneralError('Ocorreu um erro ao tentar entrar na sua conta.')
    }
  }, [searchParams])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const callbackUrl = searchParams.get('callbackUrl') || '/buscar'
  ...
  return (
    <div className="bg-slate-900/40 backdrop-blur-xl border border-slate-800/80 rounded-2xl p-6 sm:p-8 shadow-xl shadow-slate-950/50">
      <div className="mb-6">
        ...
        <p className="text-sm text-slate-400 mt-1">Insira suas credenciais para acessar a plataforma</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <div className="flex justify-between items-center mb-1.5">
            ...
            <Link href="/auth/recuperar-senha" className="text-xs font-medium text-blue-400 hover:text-blue-300 transition-colors">
              Esqueceu a senha?
            </Link>
          </div>
      ...
      <div className="mt-6 text-center text-sm">
        ...
        <Link href="/auth/cadastro" className="font-semibold text-blue-400 hover:text-blue-300 transition-colors">
          Cadastrar
        </Link>
      </div>
    </div>
